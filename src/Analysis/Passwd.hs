...
analyzePasswdfile =
  [ Seq.fromList . parsePwdEntry <$> requireTxtS "etc/passwd",
...
analyzeUnixUsers' pwds shas grps sud rh =
  checkMultiple shas shadowUsername (MultipleShadow . Multiple "username")
  ...
  where
    ...
    analyzeUser u = ConfigInformation (ConfUnixUser withsudo) Seq.<| vulns
      where
        withsudo =
          u & uusudo
            .~ ( matchingSudos & traverse . _1 %~ view (uupwd . pwdUsername)
