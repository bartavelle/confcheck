...
data FileRule = FilePattern [Pattern FP]
              | NoRule
...
userFiles :: FP -- ^ User's home directory without trailing "/"
          -> FileRule
userFiles u = FilePattern userfiles <> ruleCase worldWritable
...
cronRule :: BS.ByteString -- ^ User name
         -> FP -- ^ Command
         -> FileRule
cronRule user commandpath = FilePattern [E commandpath] <> ruleCase (\f -> not (ownedBy "root" f || ownedBy user f))
...
defFileRules =
    [ (High, FilePattern noReadFiles <> ruleCase (not . ownedBy "root")
    ...
    , (High, FilePattern noWriteFiles <> ruleCase (not . ownedByPriv)
    ...
    , (Medium, RuleCaseF (\f -> f ^. filePerms . permsSU && not (isLink f) && not (isSocket f) && not ((f ^. filePath) `HS.member` goodSetuid))
                         StrangeSuid )
    ]
    where
...
compileRule (sev, rule) = (sev, cond)
    where
        cond = CheckCondition (Or patternCond)
        ...
        mkvp (cnd, mkv) curvuln c f = if cnd (decodeFile f)
...
executable f =   f ^. filePerms . permsOX
              || f ^. filePerms . permsUX
              || f ^. filePerms . permsGX
...
data CompiledPatterns = CompiledPatterns { _cpPrefix   :: [FP]
                                         , _cpSuffix   :: [FP]
                                         , _cpInfix    :: [FP]
                                         , _cpEqua     :: [FP]
    ...
    where
        ...
        firstChecks (currem, curcheckedfiles, curchecks) c = (nextrem, newcheckedfiles, curchecks <> Seq.fromList newchecks)
            where
                ...
                nextrem = if null s && null i && r == Always False
                              then currem
                ...
                exploreFilemap ref (startidx, startfile) = goe $ if matchingP startfile then startidx else startidx + 1
                    where
                        goe i' = case CM.getIndex i' filemap of
                                    Nothing -> []
                                    Just f -> if not (matchingP f)
                                                  then []
                                                  else let nxt = goe (i' + 1)
        ...
        runChecksParent c childfile curfile = if checkCondition ($ curfile) (_ccParentcon c)
