...
getint t = BSB.foldl' parseDecimal (Right 0) t
  where
    ...
    parseDecimal (Right c) d
      | isDigitFast d = Right (fromIntegral (d - 0x30) + c * 10)
...
analyzeFS vm allvulns =
  fmap ConfigInformation (seqOf folded errors1 <> seqOf folded errors2)
    <> fileCondition allconds filemap
  where
    allconds =
      compileRules (defFileRules <> map (_2 %~ uncurry cronRule) cronstuff <> map ((Medium,) . userFiles) userhomes)
    ...
    (sudoroot, sudouser) =
      partition (checkCondition (checkUserCondition (dummyUser "root") . _sudoRunas)) $
