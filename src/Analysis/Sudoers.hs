...
cmnd :: Parser NCmnd
cmnd =
  try (NSudoEdit <$ lx (string "sudoedit"))
    <|> try (NCmndAliasName <$> name)
    <|> try (NDirectory <$> directory)
    <|> (NCommand <$> cmd)
    <?> "command alias"
  where
    directory = commandname >>= \d ->
      if T.last d == '/'
        then return d
    ...
    checkFullyqualified t
      | T.null t = fail "Empty command ? Shoud not happen."
      | T.head t == '/' = return t
...
parseCnt cnt = zip lns $ map (parse (sudoline <* comments) "dummy") lns
  where
    ...
    hascontent t
      | "#include" `T.isPrefixOf` t = False -- TODO
      | "#" `T.isPrefixOf` t = False
      | T.null t = False
      | otherwise = True
    regrouplines (a : b : xs)
      | "\\" `T.isSuffixOf` a = regrouplines (T.init a <> b : xs)
  ...
  fmap (simplifyCond1 . And) $ forM (F.toList specs) $ \(SUserSPec hosts commands) -> do
    ...
    cmdconds <- forM (propagateDefaults mempty $ F.toList commands) $ \(SCmndSpec (RunasSpec runasusers runasgroups) tags ncmnd) -> do
      ...
      let rcmd y = case y of
            ...
            Positive x -> x
          passSituation
            | NOPASSWD `S.member` tags = SudoNoPassword
...
anasudo = gensudorules . snd <$> filterTxt isSudoers
  where
    ...
    isSudoers _ = False
    gensudorules cnt =
      let (parseErrors, goodParse) =
            parseCnt cnt & traverse %~ distributeEither
              & partitionEithers
