...
checks =
  [ (S ".arp_accept", "0", Low, Just "Gratuitous ARP should not be accepted."),
...
checkSysctl key value = either Just (const Nothing) (mapM_ checkSysctl' checks)
  where
    checkSysctl' (pattrn, expected, severity, desc) =
      when
        (match pattrn key && value /= expected)
